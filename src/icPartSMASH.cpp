...
// set up IC with particles under mT cut,
// others stay in queue
// based on dynamical IC, now in hyperbolic CS
void IcPartSMASH::setIC(Fluid* f, EoS* eos, queue<Particle>* particles, double _pCut) {
 double E {0.0}, Px {0.0}, Py {0.0}, Pz {0.0};
 double Nb {0.0}, Nq {0.0}, Ns {0.0}, S {0.0};
 double Q[7], e, p, nb, nq, ns, vx, vy, vz;
 double weight;
 long unsigned int particleNumber {particles->size()};
 
 for (int i = 0; i <= particleNumber; i++) {
  
  if (particles->size() < 1) { break; cout << "BREAK!" << endl; }
  Particle particleToSmooth = particles->front();
  
  double mt = particleToSmooth.getMt(); 
  ...
  if (mt < _pCut) {
   ...
   particles->pop();
   particles->push(particleToSmooth);
  }
 }
 // all particles for IC should be in now

cout << "cell values: " << tau0 << endl;
// initialize cell values
 for (int ix = 0; ix < nx; ix++)
  for (int iy = 0; iy < ny; iy++)
   for (int iz = 0; iz < nz; iz++) {
    Q[T_] = T00[ix][iy][iz] / dx / dy / dz / tau0;
    Q[X_] = T0x[ix][iy][iz] / dx / dy / dz / tau0;
    Q[Y_] = T0y[ix][iy][iz] / dx / dy / dz / tau0;
    Q[Z_] = T0z[ix][iy][iz] / dx / dy / dz / tau0;
    Q[NB_] = QB[ix][iy][iz] / dx / dy / dz / tau0;
    Q[NQ_] = QE[ix][iy][iz] / dx / dy / dz / tau0;
    Q[NS_] = QS[ix][iy][iz] / dx / dy / dz / tau0;
    //if (Q[T_] > 0.0001) cout << ix << " " << iy << " " << iz << " " << Q[T_] << endl;
    transformPV(eos, Q, e, p, nb, nq, ns, vx, vy, vz);
...
velocityVector velocityHyperbolic(double _mt, double _px, double _py, double _y, double _eta, double _etaDiff, double _tau){
    ...
    double pTau = _mt * cosh(_y - _eta + _etaDiff);
    
    double vt = pTau / mass;
    double vx = _px / mass;
    double vy = _py / mass;
    double vz = pEta / mass;
    velocityVector v {vt, vx, vy, vz};
